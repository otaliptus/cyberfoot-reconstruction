...
run(['bash', '-c', 'source "$1/emsdk_env.sh" && make -C "$2/project/emscripten" -j "$3" BUILD_DIR=Build/MemoryFast EXTRA_CPP_FLAGS="-DBOXEDWINE_MULTI_THREADED -pthread -flto" EXTRA_LD_FLAGS="-pthread -sPTHREAD_POOL_SIZE=12 -O3 -flto" SHELL_FILE=shell.html', 'build', str(SDK), str(SOURCE), jobs])
