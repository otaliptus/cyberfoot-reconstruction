...
Implementation commit `77cbcc5` was deployed as `c833f298` to https://cyberfoot-original-emulator.pages.dev/. Public verification confirms the root launches with sound enabled, the requested header is present, desktop settings and fresh mobile startup dismiss their loading indicators, and the threaded runtime is isolated in mode3. The public HTML, loading/shell scripts, audio overlay, JS/WASM and corresponding source match local bytes. The versioned runtime and overlay are requested during play; the optional source archive is not. Browser checks report no errors, and public desktop/mobile screenshots are visually inspected. Full-match audio evidence is from the identical local runtime.
